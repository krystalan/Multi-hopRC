...
（4）阅读器     
利用多任务学习来训练阅读器：①对于每条路径进行重排序、②阅读理解，在最有可能出现答案的路径上抽取出答案。  
①对于路径选择任务来说，也是利用了BERT，输入问题q与一条路径（所有文档concat）给BERT，然后利用[CLS]获取整条路径的表示，接着通过分类任务预测出这条路径的重要程度。训练数据的话是通过构建负例来训练的。        
...
### 4.如何在一开始针对于一个复杂问题序列地获取子问题对应的答案呢？
...
***
## ICLR 2020：Neural Module Networks for Reasoning Over Text
这项工作真的非常的硬核，设计了10个模组来解决DROP数据集中的问题     
| 模组 | 输入 | 输出 | 任务|
| :---: | :---: | :---: | :---: |
| find | Question | Paragraph | 对于问题中的一个span，在文档中找到相似的span |
| filter | Question, Paragraph | Paragraph | 基于问题，在输入的span集合中选择一个子集 |
| relocate | Q, P | P | 对于输入的Paragraph Span找到论点中提出的问题 |
| find-num | P | Number | 找到输入Paragraph Span中的数字 | 
| find-date | P | Date | 找到输入Paragraph Span中的日期 |
| count | P | Count_Number | 对于输入Paragraph Span计数 |
| compare-num-lt | P, P | P | 输出有更小数字相关联的Span |
| time-diff | P, P | Time_Delta | 两个输入Paragraph Span中时间的差距 |
| find-max-num | P | P | 选择包含最大数字的Span |
| Span | P | S | 从出席的token中识别一个连续的Span |

***
## ACL 2019：Compositional Questions Do Not Necessitate Multi-hop Reasoning
### 1.研究
作者在`HotpotQA`上做了一个研究，发现很多多跳问题其实能直接用单跳模型就解决。   
然后作者又探究了为什么会有这样的现象，`HotpotQA`有两类问题：Bridge和Compare，其中Compare类确实很难用单跳QA模型来回答，但是Bridge就不一样了，作者分析了一下那些能被单跳QA模型回答的Bridge多跳问题，并将他们分成了四类：   
（1）Multi-hop（27%）：这类问题其实是必须通过多跳来解决的，例如问你“RH教授所在的大学在哪所城市？”，但是distractor中给的文档其实包含了很多所大学，所以必须要产生推理。当然单跳模型也有可能能回答对问题，因为可以靠猜嘛，但这样的策略并不在我们的考虑范畴。    
（2）Weak Distractors（35%）：这类问题其实就是按照实体类型来匹配的，因为实体类型其实是一个很强的信号。例如问你“Kiss and Tell电影里A的饰者在政府部门里担任什么职位？” 但在提供的10个distractor中只有一个女演员在政府部门有职位，所以单跳就可以直接解决，但如果是full wiki设定的情况下就不能被单跳模型来回答了。      
（3）Redundant Evidence（26%）：例如一个问题“A公司的哪个创始人现在被称为是钢铁大王？”。但这个问题最吊的地方就在于A公司的创始人就一个！什么“钢铁大王”的限制并没有什么卵用，相当于是一个单跳的问题，也就是这个问题本身就有冗余，去除掉冗余之后，实际上就是一个单跳问题。     
（4）Non-compositional Single-hop（8%）：问题本身就直接是单跳的。   

### 2.针对于Weak Distractor的研究  
因为`Weak Distractors`在full wiki的设置下可能就不能被单跳模型推理出来了，所以作者又做了更深一步探讨。然后作者分别用了两种方法替换进来干扰度更强的distractor。发现单跳模型的效果骤降，但是利用新引入进来的数据重新训练单跳模型，发现单跳模型在挣扎，虽然没有一开始的效果好，但还是进步了一些的。  

***
## NAACL 2019：Understanding Dataset Design Choices for Multi-hop Reasoning
本文在两个多跳阅读理解数据集（`HotpotQA`与`Wikihop`）上进行研究
### 1.单句包含答案实验
首先做了一个简单的实验，给模型输入一个句子以及问题，模型输出该句包含问题对应答案的概率。然后在这两个数据集上分别训练了这么一个模型，再测试，发现超过一半的样例都成功判断出了包含答案的句子，这样作者觉得很神奇，这根本就没有推理的成分嘛，因为句子与句子之间是独立的。    
### 2.可以可以不看文章直接答
那这个方法肯定说的是`Wikihop`因为`HotpotQA`是一个span式的数据集，`Wikihop`则是一个多选的数据集。我们直接不给模型文章，然后只给模型问题以及候选答案集合，让模型回答。结果他们发现60%的问题都答对了。于是作者心里想：这怎么玩？这玩什么？感觉没得玩啊。
### 3.Span-based vs. Multiple-choice
作者脑洞打开，还做了一个非常精彩的实验，`HotpotQA`是一个span式的数据集，而`Wikihop`则是一个多选的数据集。作者把`hotpotQA`(distractor setting)中的不包含答案的其余9个文档中抽取出一个实体，这9个实体和答案组成候选答案集合，以及来将`HotpotQA`改造成多选数据集。然后对于`Wikihop`数据集，作者也将其改造成了span式的数据集，具体的改造方法是：把所有文档连接起来，第一次出现答案的位置就是gold span。做了实验之后发现，在`HotpotQA`和`Wikihop-Span`上得分都比较低，而在`HotpotQA-MC`和`Wikihop`上的结果都挺高的。作者又经过分析得到了以下结论：    
